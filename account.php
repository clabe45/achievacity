<?php
	...
	if (isset($_SESSION['LoggedIn']) && $_SESSION['LoggedIn'] == 1):
		...
		if (isset($_GET['email'])) {
			if ($_GET['email'] == 'changed')
				echo "<div class='message good'>Your email has been changed.</div>";
			else if ($_GET['email'] == 'failed')
				echo "<div class='message bad'>There was an error in changing your email!</div>";
		}

		if (isset($_GET['password'])) {
			if ($_GET['password'] == 'changed')
				echo "<div class='message good'>Your password has been changed.</div>";
			// else if ($_GET['password'] == 'nomatch')	DOESN'T MAKE SENSE rn
			// 	echo "<div class='message bad'>The two passwords did not match. Try again!</div>";
			else if ($_GET['password'] == 'failed')
				echo "<div class='message bad'>There was an error in changing your password!</div>";
		}

		if (isset($_GET['delete']) && $_GET['delete'] == 'failed') {
			echo "<div class='message bad'>There was an error in deleting your account!</div>";
		}
...
<form method="post" action="db-interaction/users.php" id="change-password-form">
	<div>
		...
		<br><br>
		<input type="submit" name="change-password" id="change-password-submit" value="Change Password" class="button" />
	</div>
</form>
<hr />
